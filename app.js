...
/* A repayment you SENT to someone is money going out. The transactions table has no direction column,
   so the direction lives on the matching reimbursements row; copy it onto the in-memory transaction. */
function mbOut(t){return !!t&&t.type==='reimbursement'&&t.direction==='sent'}
function mbTagReimbursementDirections(){const d=new Map((state.reimbursements||[]).map(r=>[String(r.transaction_id),r.direction||'received']));for(const t of state.transactions||[]){if(t.type==='reimbursement'){const v=d.get(String(t.id));if(v)t.direction=v}}}
function rebuildIndexes(){mbTagReimbursementDirections();txCategoryIndex=new Map();for(const x of state.transaction_categories||[]){if(!txCategoryIndex.has(x.transaction_id))txCategoryIndex.set(x.transaction_id,[]);txCategoryIndex.get(x.transaction_id).push(x)}txAccountIndex=new Map();for(const x of state.transaction_accounts||[]){if(!txAccountIndex.has(x.transaction_id))txAccountIndex.set(x.transaction_id,[]);txAccountIndex.get(x.transaction_id).push(x)}}
...
function accountBalance(a){let b=Number(a.opening_balance||0);for(const t of state.transactions){const n=Number(t.amount||0);if(t.type==='transfer'){if(t.account_id===a.id)b-=n;if(t.to_account_id===a.id)b+=n}else if(t.account_id===a.id){if(t.type==='income'||(t.type==='reimbursement'&&!mbOut(t)))b+=n;if(t.type==='expense'||t.type==='split'||mbOut(t))b-=n}}for(const l of state.loans){const n=Number(l.amount||0);if(l.account_id===a.id)b+=l.direction==='borrow'?n:-n}for(const r of state.loan_repayments){const n=Number(r.amount||0);if(r.account_id===a.id)b+=r.direction==='received'?n:-n}for(const h of state.money_held){const n=Number(h.amount||0);if(h.account_id===a.id)b+=h.status==='pending'?n:-n}return b}
...
function txHTML(t){const icon={income:'↑',expense:'−',transfer:'⇄',split:'🔀',reimbursement:'↩'}[t.type]||'•',sign=t.type==='income'||(t.type==='reimbursement'&&!mbOut(t))?'+':t.type==='expense'||t.type==='split'||mbOut(t)?'−':'',other=t.type==='transfer'?` → ${esc(accountName(t.to_account_id))}`:'',share=t.type==='split'?` · ${esc(getNickname())} share ${money(splitMyShare(t))}`:'',cats=txAllocations(t).map(a=>catName(a.category_id)).filter(Boolean);return `<div class="row transaction-row"><div class="left"><div class="bubble ${t.type==='income'||(t.type==='reimbursement'&&!mbOut(t))?'income':(mbOut(t)?'expense':t.type)}">${icon}</div><div class="tx-content"><div class="name">${esc(t.description||'(No description)')}</div><div class="sub">${fmtDate(t.transaction_date)} · ${esc(accountAllocationSummary(t))}${other}${share}</div>${cats.length?`<div class="sub">Category: ${esc(cats.join(', '))}</div>`:''}${t.notes?`<div class="sub">${esc(t.notes)}</div>`:''}</div></div><div class="tx-right" style="text-align:right"><b class="${sign==='+'?'green':sign==='−'?'red':''}">${sign}${money(t.amount)}</b><div class="action-row"><button class="smallbtn" onclick="editTx('${t.id}')">Edit</button><button class="smallbtn dangerbtn" onclick="deleteTx('${t.id}')">Delete</button></div></div></div>`}
async function submitForm(type,data,f){const btn=f.querySelector('.primary');if(btn)btn.disabled=true;try{await saveModal(type,data,f);await mbLoadForMutation(mbMutationTables(type));closeModal();render();mbToast(type==='budget'?(data?'Budget updated.':'Budget added.'):type==='goal'?(data?'Goal updated.':'Goal added.'):(data?'Updated successfully.':'Saved successfully.'))}catch(e){console.error('[My Budget] Save failed:',e);mbToast(friendlyError(e),'error')}finally{if(btn)btn.disabled=false}}
...
(function(){
  ...
  function accountEffects(t){
    ...
    if(t.__special==='loan'){if(t.account_id)out.push([t.account_id,t.direction==='borrow'?n:-n]);}
      ...
      if(allocs.length && (t.type==='income'||t.type==='reimbursement'||t.type==='expense'||t.type==='split')){
        ...
        allocs.forEach(x=>{const share=Number(x.amount||0);const delta=(t.type==='income'||(t.type==='reimbursement'&&!mbOut(t))?1:-1)*(total?share*n/total:0);out.push([x.account_id,delta]);});
      } else if(t.account_id){
        if(t.type==='income'||(t.type==='reimbursement'&&!mbOut(t)))out.push([t.account_id,n]);
        else if(t.type==='expense'||t.type==='split'||mbOut(t))out.push([t.account_id,-n]);
      }
    }
    return out;
  }
  ...
  window.txHTML=function(t){
    ...
    const icon={income:'↑',expense:'−',transfer:'⇄',split:'🔀',reimbursement:'↩'}[t.type]||'•', sign=t.type==='income'||(t.type==='reimbursement'&&!mbOut(t))?'+':t.type==='expense'||t.type==='split'||mbOut(t)?'−':'', other=t.type==='transfer'&&t.to_account_id?` → ${esc(accountName(t.to_account_id))}`:'', share=t.type==='split'?` · ${esc(getNickname())} share ${money(splitMyShare(t))}`:'', cats=txAllocations(t).map(a=>catName(a.category_id)).filter(Boolean);
    return `<div class="row transaction-row"><div class="left"><div class="bubble ${t.type==='income'||(t.type==='reimbursement'&&!mbOut(t))?'income':(mbOut(t)?'expense':t.type)}">${icon}</div><div class="tx-content"><div class="name">${esc(t.description||'(No description)')}</div><div class="sub">${esc(accountAllocationSummary(t))}${other}${share}</div>${cats.length?`<div class="sub">Category: ${esc(cats.join(', '))}</div>`:''}${stamp}${balanceHTML(t)}${t.notes?`<div class="sub">${esc(t.notes)}</div>`:''}</div></div><div class="tx-right" style="text-align:right"><b class="${sign==='+'?'green':sign==='−'?'red':''}">${sign}${money(t.amount)}</b><div class="action-row">${actions(t)}</div></div></div>`;
  };
...
(function(){
  ...
  function ledgerEffects(t){
    ...
    if(alloc.length){const total=alloc.reduce((s,x)=>s+Number(x.amount||0),0)||n;alloc.forEach(x=>{const share=Number(x.amount||0);const sign=(t.type==='income'||(t.type==='reimbursement'&&!mbOut(t)))?1:-1;out.push([x.account_id,sign*(total?share*n/total:0)])});return out;}
    if(t.account_id){if(t.type==='income'||(t.type==='reimbursement'&&!mbOut(t)))out.push([t.account_id,n]);else if(t.type==='expense'||t.type==='split'||mbOut(t))out.push([t.account_id,-n]);}
    return out;
  }
...
(function(){
  ...
  window.saveModal=async function(type,data,f){
    ...
    let available=direction==='sent'?Number(bal.iOwe||0):Number(bal.balance||0);
    // Editing an existing repayment: its own amount is already counted as paid, so give it back first.
    if(type==='repaymentEdit'&&data?.reimbursement){const o=data.reimbursement;if(String(o.person_id)===String(x.person_id)&&(o.direction==='sent'?'sent':'received')===direction)available+=Number(o.amount||0)}
...
(function(){
  ...
  function effects(t){
    ...
    const alloc=(state.transaction_accounts||[]).filter(x=>x.transaction_id===t.id&&x.account_id);
    if(alloc.length){const total=alloc.reduce((s,x)=>s+Number(x.amount||0),0)||n;return alloc.map(x=>[x.account_id,(t.type==='income'||(t.type==='reimbursement'&&!mbOut(t))?1:-1)*Number(x.amount||0)*n/total]);}
    if(t.account_id)return [[t.account_id,(t.type==='income'||(t.type==='reimbursement'&&!mbOut(t)))?n:-n]];
    return [];
  }
  function balanceMap(rows){
  ...
  function rowHTML(t,balances){
    ...
    const sign=t.__special==='held_settlement'?'−':((t.__special==='loan'&&t.direction==='lend')||(t.__special==='loan_repayment'&&t.direction==='sent')?'-':(t.__special==='loan'&&t.direction==='borrow')||(t.__special==='loan_repayment'&&t.direction==='received')?'+':(t.type==='income'||(t.type==='reimbursement'&&!mbOut(t))?'+':t.type==='expense'||t.type==='split'||mbOut(t)?'−':''));
...
(function(){
  ...
  const iconClass=t=>{
    ...
    if(t.type==='reimbursement')return mbOut(t)?'expense':'income';
    return 'split';
  };
  window.txHTML=function(t){
    ...
    const icon=iconFor(t), cls=iconClass(t), sign=(t.__special==='loan'?(t.direction==='borrow'?'+':'−'):t.__special==='loan_repayment'?(t.direction==='received'?'+':'−'):t.__special==='held'?' +':t.__special==='held_settlement'?'−':t.type==='income'||(t.type==='reimbursement'&&!mbOut(t))?'+':t.type==='expense'||t.type==='split'||mbOut(t)?'−':'');
    let meta='';
